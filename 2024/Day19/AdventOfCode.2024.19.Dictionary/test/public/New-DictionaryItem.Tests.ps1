Describe "Testing public module function New-DictionaryItem" -Tag "UnitTest" {
    BeforeEach {
    ...
    Context "A dummy unit test" {
        ...
        It "Should not output data" {
            $Result = Invoke-ModuleFunctionStub -Key xyz
            $Result | Should -BeNullOrEmpty
        }

        It "Should add the Key to the dictionary" {
            Invoke-ModuleFunctionStub -Key xyz
            $script:AOC_2024_19_DICTIONARY.ContainsKey('xyz') | Should -BeTrue
            $script:AOC_2024_19_DICTIONARY['xyz'].Pattern | Should -Be 'xyz'
            Should -ActualValue $script:AOC_2024_19_DICTIONARY['xyz'].Parts -BeOfType 'System.Collections.ArrayList'
            $script:AOC_2024_19_DICTIONARY['xyz'].Parts.Count | Should -Be 0
        }
    }
}
