...
Process {
    ...
    $InputData | ForEach-Object {
        ...
        for($i = 0; $i -lt 100; $i ++) {
            ...
            $MulA = $_.A.PSObject.Copy()
            $MulA.X *= $i
            $MulA.Y *= $i

            $MulB = $_.B.PSObject.Copy()
            $MulB.X *= $i
            $MulB.Y *= $i

            $SolA = $_.Prize.PSObject.Copy()
            $SolA.X -= $MulA.X
            $SolA.Y -= $MulA.Y

            $SolB = $_.Prize.PSObject.Copy()
            $SolB.X -= $MulB.X
            $SolB.Y -= $MulB.Y
            ...
            if($SolA.X % $_.B.X -eq 0) {
                $N = $SolA.X / $_.B.X
                if($SolA.Y / $_.B.Y -eq $N) {
                    # Write-Warning "Solution found: A=$i, B=$N"
                    $Solutions += ,([pscustomobject]@{
                        A = $i
                        B = $N
                    })
            ...
            if($SolB.X % $_.A.X -eq 0) {
                $N = $SolB.X / $_.A.X
                if($SolB.Y / $_.A.Y -eq $N) {
                    # Write-Warning "Solution found: A=$N, B=$i"
                    $Solutions += ,([pscustomobject]@{
                        A = $N
                        B = $i
                    })
