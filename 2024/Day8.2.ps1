    ...
    $Point | select-object -ExpandProperty freq -Unique | Foreach-Object {
        ...
        for($i=0;$i -lt $set.count - 1;$i++){
            ...
            for($j=1;$j -lt $set.count -$i;$j++){
                ...
                $n = 0
                while($true) {
                    $U = [pscustomobject]@{x=$A.x - ($n * $T.x); y=$A.y - ($n * $T.y)} | Write-Output
                    $U | Write-Output
                    if(-not($U.x -ge $Origin.x -and $U.x -le $End.x -and $U.y -ge $Origin.y -and $U.y -le $End.y)){
                        break;
                    }
                    $n++
                }
                $n = 0
                while($true) {
                    $U = [pscustomobject]@{x=$B.x + ($n * $T.x); y=$B.y + ($n * $T.y)} | Write-Output
                    $U | Write-Output
                    if(-not($U.x -ge $Origin.x -and $U.x -le $End.x -and $U.y -ge $Origin.y -and $U.y -le $End.y)){
                        break;
                    }
